...
class DynamicPanelDGP(_BaseDynamicPanelDGP):
    ...
    def create_instance(
        ...
        for t in range(self.n_x):
            self.Beta[t, :] = autoreg * np.roll(
                np.random.uniform(low=4.0 ** (-np.arange(0, self.n_x)), high=4.0 ** (-np.arange(1, self.n_x + 1))), t
            )
        if self.hetero_inds is not None:
        ...
        if self.hetero_inds is not None:
            self.y_hetero_effect[self.hetero_inds] = np.random.uniform(
                0.5 * hetero_strength, 1.5 * hetero_strength
            ) / len(self.hetero_inds)
            self.x_hetero_effect[self.hetero_inds] = np.random.uniform(
                0.5 * hetero_strength, 1.5 * hetero_strength
            ) / len(self.hetero_inds)
        ...
        for t in np.arange(self.n_periods - 2, -1, -1):
            self.true_effect[t, :] = (
                self.zeta.reshape(1, -1) @ np.linalg.matrix_power(self.Beta, (self.n_periods - 1 - t) - 1) @ self.Alpha
            )
    ...
    def _gen_data_with_policy(self, n_units, policy_gen, random_seed=123):
        ...
        for t in range(n_units * self.n_periods):
            ...
            Y[t] = (
                (np.dot(self.y_hetero_effect, X_t if period != 0 else X[t]) + 1) * np.dot(self.epsilon, T[t])
    ...
    def observational_data(self, n_units, gamma=0, s_t=1, sigma_t=0.5, random_seed=123):
        ...
        def policy_gen(Tpre, X, period):
            return gamma * Tpre + (1 - gamma) * np.dot(Delta, X) + np.random.normal(0, sigma_t, size=self.n_treatments)
...
def add_vlines(n_periods, n_treatments, hetero_inds):
    ...
    locs += [-0.501 + (len(hetero_inds) + 1) / 2]
    labels += ["\n\n$\\tau_{{{}}}$".format(0)]
    ...
    for q in np.arange(1, n_treatments):
        plt.axvline(x=q * (len(hetero_inds) + 1) - 0.5, linestyle='--', color='red', alpha=0.2)
        locs += [q * (len(hetero_inds) + 1) - 0.501 + (len(hetero_inds) + 1) / 2]
        labels += ["\n\n$\\tau_{{{}}}$".format(q)]
        locs += [(q * (len(hetero_inds) + 1) + qx) for qx in np.arange(len(hetero_inds) + 1)]
        labels += ["$1$"] + ["$x_{{{}}}$".format(qx) for qx in hetero_inds]
    locs += [-0.501 + (len(hetero_inds) + 1) * n_treatments / 2]
    labels += ["\n\n\n\n$\\theta_{{{}}}$".format(0)]
    for t in np.arange(1, n_periods):
        plt.axvline(x=t * (len(hetero_inds) + 1) * n_treatments - 0.5, linestyle='-', alpha=0.6)
        locs += [t * (len(hetero_inds) + 1) * n_treatments - 0.501 + (len(hetero_inds) + 1) * n_treatments / 2]
        labels += ["\n\n\n\n$\\theta_{{{}}}$".format(t)]
        locs += [t * (len(hetero_inds) + 1) * n_treatments - 0.501 + (len(hetero_inds) + 1) / 2]
        labels += ["\n\n$\\tau_{{{}}}$".format(0)]
        locs += [t * (len(hetero_inds) + 1) * n_treatments + qx for qx in np.arange(len(hetero_inds) + 1)]
        labels += ["$1$"] + ["$x_{{{}}}$".format(qx) for qx in hetero_inds]
        for q in np.arange(1, n_treatments):
            plt.axvline(
                x=t * (len(hetero_inds) + 1) * n_treatments + q * (len(hetero_inds) + 1) - 0.5,
                linestyle='--',
                color='red',
                alpha=0.2,
            )
            locs += [
                t * (len(hetero_inds) + 1) * n_treatments
                + q * (len(hetero_inds) + 1)
                - 0.501
                + (len(hetero_inds) + 1) / 2
            ]
            labels += ["\n\n$\\tau_{{{}}}$".format(q)]
            locs += [
                t * (len(hetero_inds) + 1) * n_treatments + (q * (len(hetero_inds) + 1) + qx)
