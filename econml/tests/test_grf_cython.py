...
class TestGRFCython(unittest.TestCase):
    ...
    def _get_cython_objects(
        ...
        *,
        ...
        if criterion == 'het':
            criterion = LinearMomentGRFCriterion(
                n_outputs, n_relevant_outputs, n_features, n_y, n_samples, max_node_samples, random_state
            )
            criterion_val = LinearMomentGRFCriterion(
                n_outputs, n_relevant_outputs, n_features, n_y, n_samples, max_node_samples, random_state
            )
        else:
            criterion = LinearMomentGRFCriterionMSE(
                n_outputs, n_relevant_outputs, n_features, n_y, n_samples, max_node_samples, random_state
            )
            criterion_val = LinearMomentGRFCriterionMSE(
                n_outputs, n_relevant_outputs, n_features, n_y, n_samples, max_node_samples, random_state
            )
    ...
    def _get_continuous_data(self, config):
        ...
        return X, yaug, np.hstack([(X[:, [0]] > 0.0) + 0.5, (X[:, [0]] > 0.0) + 0.5])
    ...
    def _get_true_quantities(self, config, X, y, mask, criterion):
        ...
        pointJ = y[
            mask, config['n_y'] + config['n_outputs'] : config['n_y'] + (config['n_outputs'] + 1) * config['n_outputs']
        ]
    ...
    def _test_tree_quantities(self, base_config_gen, criterion):
        ...
        if config['honest']:
            np.testing.assert_array_equal(
                config['samples_val'], np.arange(config['n_samples_train'], 2 * config['n_samples_train'])
            )
        np.testing.assert_array_equal(tree.feature, np.array([0, -2, -2]))
        np.testing.assert_allclose(tree.threshold, np.array([0, -2, -2]), atol=0.1, rtol=0)
        [
            np.testing.assert_allclose(a, b, atol=1e-4)
            for a, b in zip(
                self._get_true_quantities(config, X, y, np.ones(X.shape[0]) > 0, criterion),
        ...
        [
            np.testing.assert_allclose(a, b, atol=1e-4)
            for a, b in zip(
                self._get_true_quantities(config, X, y, X[:, tree.feature[0]] < tree.threshold[0], criterion),
        ...
        [
            np.testing.assert_allclose(a, b, atol=1e-4)
            for a, b in zip(
                self._get_true_quantities(config, X, y, mask0, criterion), self._get_node_quantities(tree, 1)
            )
        ]
        [
            np.testing.assert_allclose(a, b, atol=1e-4)
            for a, b in zip(
                self._get_true_quantities(config, X, y, ~mask0, criterion), self._get_node_quantities(tree, 4)
            )
        ]
        ...
        [
            np.testing.assert_allclose(a, b, atol=1e-4)
            for a, b in zip(
                self._get_true_quantities(config, X, y, mask1a, criterion), self._get_node_quantities(tree, 2)
            )
        ]
        ...
        [
            np.testing.assert_allclose(a, b, atol=1e-4)
            for a, b in zip(
                self._get_true_quantities(config, X, y, mask1b, criterion), self._get_node_quantities(tree, 3)
            )
        ]
        ...
        [
            np.testing.assert_allclose(a, b, atol=1e-4)
            for a, b in zip(
                self._get_true_quantities(config, X, y, mask1c, criterion), self._get_node_quantities(tree, 5)
            )
        ]
        ...
        [
            np.testing.assert_allclose(a, b, atol=1e-4)
            for a, b in zip(
                self._get_true_quantities(config, X, y, mask1d, criterion), self._get_node_quantities(tree, 6)
            )
        ]
    ...
    def test_honest_dishonest_equivalency(self):
        for criterion in ['het', 'mse']:
            ...
            np.testing.assert_equal(
                tree.compute_feature_heterogeneity_importances(),
    ...
    def test_min_var_leaf(self):
        ...
        for criterion in ['het', 'mse']:
            ...
            config['min_eig_leaf'] = 0.2
            config['criterion'] = criterion
            X = np.arange(n_samples_train).reshape(-1, 1)
            T = np.random.binomial(1, 0.5, size=(n_samples_train, 1))
    ...
    def test_fast_eigv(self):
        ...
        for _ in range(10):
            ...
            np.testing.assert_allclose(apx, opt, atol=0.01, rtol=0.3)
            apx = fast_max_eigv(A, 10, 123)
            opt = np.max(np.linalg.eig(A)[0])
            np.testing.assert_allclose(apx, opt, atol=0.5, rtol=0.2)
    ...
    def test_linalg(self):
        ...
        for n, m, nrhs in [(3, 3, 3), (3, 2, 1), (3, 1, 2), (1, 4, 2), (3, 4, 5)]:
            for _ in range(100):
                ...
                np.testing.assert_allclose(ours, true, atol=0.00001, rtol=0.0)
