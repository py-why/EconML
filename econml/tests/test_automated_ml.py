...
class TestDML(unittest.TestCase):
    ...
    def test_param(self):
        ...
        (W, X, Y, T) = [make_random(is_discrete, d) for (is_discrete,
                                                         d) in [(False, 2), (False, 2), (False, 0),
                                                                (True, 0)]]
        for estimator in [AutomatedKernelDMLCateEstimator,
                          AutomatedSparseLinearDMLCateEstimator,
                          AutomatedKernelDMLCateEstimator]:
    ...
    def test_forest_dml(self):
        ...
        X_sum = np.vstack([np.array(X1_sum)[:, 1:], np.array(X2_sum)[:,
                                                                     1:]])
        T_sum = np.concatenate((np.array(X1_sum)[:, 0],
                                np.array(X2_sum)[:, 0]))
        ...
        est = AutomatedForestDMLCateEstimator(
    ...
    def test_can_use_vectors(self):
        ...
        dmls = \
            [AutomatedLinearDMLCateEstimator(model_y=automl_model_sample_weight_reg(),
                                             model_t=automl_model_sample_weight_reg(),
                                             fit_cate_intercept=False),
             AutomatedSparseLinearDMLCateEstimator(model_y=automl_model_sample_weight_reg(),
                                                   model_t=automl_model_sample_weight_reg(),
                                                   fit_cate_intercept=False)]
        for dml in dmls:
            dml.fit(np.array([
    ...
    def test_can_use_sample_weights(self):
        ...
        dmls = \
            [AutomatedLinearDMLCateEstimator(model_y=automl_model_reg(),
                                             model_t='auto', fit_cate_intercept=False),
             AutomatedSparseLinearDMLCateEstimator(model_y=automl_model_reg(),
                                                   model_t='auto', fit_cate_intercept=False)]
        for dml in dmls:
            dml.fit(np.array([
                1,
    ...
    def test_discrete_treatments(self):
        ...
        dmls = \
            [AutomatedLinearDMLCateEstimator(model_y=automl_model_reg(),
                                             model_t=automl_model_clf(), fit_cate_intercept=False,
                                             discrete_treatment=True),
             AutomatedSparseLinearDMLCateEstimator(model_y=automl_model_reg(),
                                                   model_t=automl_model_clf(), fit_cate_intercept=False,
                                                   discrete_treatment=True)]
...
class TestMetalearners(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.X_test = \
            cls.random_state.multivariate_normal(np.zeros(cls.d),
                                                 np.diag(np.ones(cls.d)), cls.n_test)

        # Constant treatment effect

        cls.const_te_data = TestMetalearners._generate_data(cls.n,
                                                            cls.d, beta=cls.random_state.uniform(0, 1, cls.d),
                                                            treatment_effect=TestMetalearners._const_te,
                                                            multi_y=False)

        # Constant treatment with multi output Y

        cls.const_te_multiy_data = \
            TestMetalearners._generate_data(cls.n, cls.d,
                                            beta=cls.random_state.uniform(0, 1, size=(cls.d, 2)),
                                            treatment_effect=TestMetalearners._const_te,
                                            multi_y=True)

        # Heterogeneous treatment

        cls.heterogeneous_te_data = \
            TestMetalearners._generate_data(cls.n, cls.d,
                                            beta=cls.random_state.uniform(0, 1, cls.d),
                                            treatment_effect=TestMetalearners._heterogeneous_te,
                                            multi_y=False)

        # Heterogeneous treatment with multi output Y

        cls.heterogeneous_te_multiy_data = \
            TestMetalearners._generate_data(cls.n, cls.d,
                                            beta=cls.random_state.uniform(0, 1, size=(cls.d, 2)),
                                            treatment_effect=TestMetalearners._heterogeneous_te,
                                            multi_y=True)
    ...
    def test_SLearner(self):
        ...
        overall_model = Pipeline([('poly', PolynomialFeatures()),
                                  ('model', automl_model_reg())])
    ...
    def _test_te(
        ...
        te_func = getattr(TestMetalearners,
                          '_{te_type}_te'.format(te_type=te_type))
        if multi_y:
            (X, T, Y) = getattr(TestMetalearners,
                                '{te_type}_te_multiy_data'.format(te_type=te_type))

            # Get the true treatment effect

            te = np.repeat((np.apply_along_axis(te_func, 1,
                                                TestMetalearners.X_test) * (T1 - T0)).reshape(-1, 1), 2, axis=1)
            marginal_te = np.repeat(np.apply_along_axis(te_func, 1,
                                                        TestMetalearners.X_test).reshape(-1,
                                                                                         1) * np.array([2, 4]), 2,
                                    axis=0).reshape((-1, 2, 2))
        else:
            (X, T, Y) = getattr(TestMetalearners,
                                '{te_type}_te_data'.format(te_type=te_type))

            # Get the true treatment effect

            te = np.apply_along_axis(te_func, 1,
                                     TestMetalearners.X_test) * (T1 - T0)
            marginal_te = np.apply_along_axis(te_func, 1,
                                              TestMetalearners.X_test).reshape(-1, 1) \
                * np.array([2, 4])
        ...
        te_hat = learner_instance.effect(TestMetalearners.X_test,
                                         T0=T0, T1=T1)
        marginal_te_hat = learner_instance.marginal_effect(T1,
                                                           TestMetalearners.X_test)
    ...
    def _generate_data(
        ...
        X = cls.random_state.multivariate_normal(np.zeros(d),
                                                 np.diag(np.ones(d)), n)

        # Generate treatment

        T = cls.random_state.choice([1, 3, 5], size=n, p=[0.2, 0.3, .5])

        # Calculate outcome

        Y0 = (np.dot(X, beta) + cls.random_state.normal(0,
                                                        1)).reshape(n, -1)
        treat_effect = np.apply_along_axis(lambda x:
                                           treatment_effect(x), 1, X)
