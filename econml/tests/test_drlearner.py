...
class TestDRLearner(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Test data
        cls.X_test = cls.random_state.multivariate_normal(np.zeros(cls.d), np.diag(np.ones(cls.d)), cls.n_test)
        # Constant treatment effect and propensity
        cls.const_te_data = TestDRLearner._generate_data(
            cls.n, cls.d, cls._untreated_outcome, treatment_effect=TestDRLearner._const_te, propensity=lambda x: 0.3
        )
        # Heterogeneous treatment and propensity
        cls.heterogeneous_te_data = TestDRLearner._generate_data(
            cls.n,
            cls.d,
            cls._untreated_outcome,
                ...
                for d_x in [2, None]:
                    for d_w in [2, None]:
                        W, X, Y, T = [
                            ...
                            for is_discrete, d in [(False, d_w), (False, d_x), (False, d_y), (is_discrete, d_t)]
                        ]
                        ...
                        marginal_effect_shape = (
                            (n,) + ((d_y,) if d_y > 0 else ()) + ((d_t_final,) if d_t_final > 0 else ())
                        )
                        marginal_effect_summaryframe_shape = (
                            n * (d_y if d_y > 0 else 1) * (d_t_final if d_t_final > 0 else 1),
                            6,
                        )

                        # since T isn't passed to const_marginal_effect, defaults to one row if X is None
                        const_marginal_effect_shape = (
                            (n if d_x else 1,) + ((d_y,) if d_y > 0 else ()) + ((d_t_final,) if d_t_final > 0 else ())
                        )
                        const_marginal_effect_summaryframe_shape = (
                            (n if d_x else 1) * (d_y if d_y > 0 else 1) * (d_t_final if d_t_final > 0 else 1),
                            6,
                        )
                        ...
                        for est in [
                            ...
                            DRLearner(
                                model_propensity=LogisticRegression(multi_class='auto'),
                            ...
                            for inf in infs:
                                with self.subTest(
                                    d_w=d_w, d_x=d_x, d_y=d_y, d_t=d_t, is_discrete=is_discrete, est=est, inf=inf
                                ):
                                    ...
                                    if inf is not None:
                                        ...
                                        self.assertEqual(
                                            shape(est.effect_interval(X, T0=T0, T1=T1)), (2,) + effect_shape
                                        )
                                        ...
                                        self.assertEqual(
                                            shape(const_marg_effect_inf.summary_frame()),
                                            const_marginal_effect_summaryframe_shape,
                                        )
                                        self.assertEqual(
                                            shape(const_marg_effect_inf.point_estimate), const_marginal_effect_shape
                                        )
                                        self.assertEqual(
                                            shape(const_marg_effect_inf.stderr), const_marginal_effect_shape
                                        )
                                        self.assertEqual(shape(const_marg_effect_inf.var), const_marginal_effect_shape)
                                        self.assertEqual(
                                            shape(const_marg_effect_inf.pvalue()), const_marginal_effect_shape
                                        )
                                        self.assertEqual(
                                            shape(const_marg_effect_inf.zstat()), const_marginal_effect_shape
                                        )
                                        self.assertEqual(
                                            shape(const_marg_effect_inf.conf_int()), (2,) + const_marginal_effect_shape
                                        )
                                        np.testing.assert_array_almost_equal(
                                            const_marg_effect_inf.conf_int()[0], const_marg_eff_int[0], decimal=5
                                        )
                                        const_marg_effect_inf.population_summary()._repr_html_()

                                        # test effect inference
                                        self.assertEqual(shape(effect_inf.summary_frame()), effect_summaryframe_shape)
                                        self.assertEqual(shape(effect_inf.point_estimate), effect_shape)
                                        self.assertEqual(shape(effect_inf.stderr), effect_shape)
                                        self.assertEqual(shape(effect_inf.var), effect_shape)
                                        self.assertEqual(shape(effect_inf.pvalue()), effect_shape)
                                        self.assertEqual(shape(effect_inf.zstat()), effect_shape)
                                        self.assertEqual(shape(effect_inf.conf_int()), (2,) + effect_shape)
                                        np.testing.assert_array_almost_equal(
                                            effect_inf.conf_int()[0], est.effect_interval(X, T0=T0, T1=T1)[0], decimal=5
                                        )
                                        effect_inf.population_summary()._repr_html_()

                                        # test marginal effect inference
                                        self.assertEqual(
                                            shape(marg_effect_inf.summary_frame()), marginal_effect_summaryframe_shape
                                        )
                                        self.assertEqual(shape(marg_effect_inf.point_estimate), marginal_effect_shape)
                                        self.assertEqual(shape(marg_effect_inf.stderr), marginal_effect_shape)
                                        self.assertEqual(shape(marg_effect_inf.var), marginal_effect_shape)
                                        self.assertEqual(shape(marg_effect_inf.pvalue()), marginal_effect_shape)
                                        self.assertEqual(shape(marg_effect_inf.zstat()), marginal_effect_shape)
                                        self.assertEqual(
                                            shape(marg_effect_inf.conf_int()), (2,) + marginal_effect_shape
                                        )
                                        np.testing.assert_array_almost_equal(
                                        ...
                                        if test_linear_attrs:
                                            if X is not None:
                                                ...
                                                self.assertEqual(
                                                    shape(coef_inf.summary_frame()), coef_summaryframe_shape
                                                )
                                                np.testing.assert_array_almost_equal(
                                            ...
                                            self.assertEqual(
                                                shape(int_inf.summary_frame()), intercept_summaryframe_shape
                                            )
                                            np.testing.assert_array_almost_equal(
    ...
    def test_can_use_sample_weights(self):
        ...
        dml.fit(np.array([1, 2, 1, 2]), np.array([1, 2, 1, 2]), W=np.ones((4, 1)), sample_weight=np.ones((4,)))
    ...
    def test_discrete_treatments(self):
        ...
        np.testing.assert_almost_equal(
            dml.effect(
                np.ones((9, 1)), T0=np.array([1, 1, 1, 2, 2, 2, 3, 3, 3]), T1=np.array([1, 2, 3, 1, 2, 3, 1, 2, 3])
            ),
            [0, 2, 1, -2, 0, -1, -1, 1, 0],
        )
    ...
    def test_can_use_statsmodel_inference(self):
        ...
        point = dml.effect(
            np.ones((9, 1)), T0=np.array([1, 1, 1, 1, 1, 1, 1, 1, 1]), T1=np.array([2, 2, 3, 2, 2, 3, 2, 2, 3])
        )
    ...
    def test_drlearner_all_attributes(self):
        ...
        sigma = 0.01
        y = (1 + 0.5 * controls[:, 0]) * T + controls[:, 0] + np.random.normal(0, sigma, size=(5000,))
        for X in [controls]:
                        ...
                        for featurizer in [None, PolynomialFeatures(degree=2, include_bias=False)]:
                            for models in [
                                ...
                                for multitask_model_final in [False, True]:
                                    if (not isinstance(models[2], StatsModelsLinearRegression)) and (
                                        sample_var is not None
                                    ):
                                        continue
                                    with self.subTest(
                                        X=X,
                                        ...
                                        np.testing.assert_allclose(
                                            est.effect(X[:3], T0=0, T1=1), 1 + 0.5 * X[:3, 0], rtol=0, atol=0.15
                                        )
                                        np.testing.assert_allclose(
                                            est.const_marginal_effect(X[:3]),
                                            np.hstack([1 + 0.5 * X[:3, [0]], 2 * (1 + 0.5 * X[:3, [0]])]),
                                            rtol=0,
                                            atol=0.15,
                                        )
                                        for t in [1, 2]:
                                            np.testing.assert_allclose(
                                                est.marginal_effect(t, X[:3]),
                                                np.hstack([1 + 0.5 * X[:3, [0]], 2 * (1 + 0.5 * X[:3, [0]])]),
                                                rtol=0,
                                                atol=0.15,
                                            )
                                        ...
                                        if featurizer is not None:
                                            out_feat_names = get_feature_names_or_default(
                                                featurizer.fit(X), feature_names
                                            )
                                            np.testing.assert_array_equal(est.featurizer_.n_features_in_, 3)
                                        np.testing.assert_array_equal(
                                            est.cate_feature_names(feature_names), out_feat_names
                                        )

                                        if isinstance(models[0], GradientBoostingClassifier):
                                            np.testing.assert_array_equal(
                                                np.array(
                                                    [mdl.feature_importances_ for mdl in est.models_regression[0]]
                                                ).shape,
                                                [2, 2 + X.shape[1] + (W.shape[1] if W is not None else 0)],
                                            )
                                            np.testing.assert_array_equal(
                                                np.array(
                                                    [mdl.feature_importances_ for mdl in est.models_propensity[0]]
                                                ).shape,
                                                [2, X.shape[1] + (W.shape[1] if W is not None else 0)],
                                            )
                                        else:
                                            np.testing.assert_array_equal(
                                                np.array([mdl.coef_ for mdl in est.models_regression[0]]).shape,
                                                [2, 2 + X.shape[1] + (W.shape[1] if W is not None else 0)],
                                            )
                                            np.testing.assert_array_equal(
                                                np.array([mdl.coef_ for mdl in est.models_propensity[0]]).shape,
                                                [2, 3, X.shape[1] + (W.shape[1] if W is not None else 0)],
                                            )
                                        if multitask_model_final:
                                            if isinstance(models[2], RandomForestRegressor):
                                                np.testing.assert_equal(
                                                    np.argsort(est.multitask_model_cate.feature_importances_)[-1], 0
                                                )
                                            else:
                                                true_coef = np.zeros((2, len(out_feat_names)))
                                                true_coef[:, 0] = [0.5, 1]
                                                np.testing.assert_allclose(
                                                    est.multitask_model_cate.coef_, true_coef, rtol=0, atol=0.15
                                                )
                                                np.testing.assert_allclose(
                                                    est.multitask_model_cate.intercept_, [1, 2], rtol=0, atol=0.15
                                                )
                                        else:
                                            for t in [1, 2]:
                                                if isinstance(models[2], RandomForestRegressor):
                                                    np.testing.assert_equal(
                                                        np.argsort(est.model_cate(T=t).feature_importances_)[-1], 0
                                                    )
                                                else:
                                                    true_coef = np.zeros(len(out_feat_names))
                                                    true_coef[0] = 0.5 * t
                                                    np.testing.assert_allclose(
                                                        est.model_cate(T=t).coef_, true_coef, rtol=0, atol=0.15
                                                    )
                                                    np.testing.assert_allclose(
                                                        est.model_cate(T=t).intercept_, t, rtol=0, atol=0.15
                                                    )
    ...
    def _test_drlearner_with_inference_all_attributes(self, use_ray):
        ...
        T = np.random.binomial(2, scipy.special.expit(controls[:, 0]))
        sigma = 0.01
        y = (1 + 0.5 * controls[:, 0]) * T + controls[:, 0] + np.random.normal(0, sigma, size=(10000,))
            ...
            for W in [None, controls]:
                for sample_weight in [None, np.ones(T.shape[0])]:
                    for freq_weight, sample_var in [(None, None), (np.ones(T.shape[0]), np.zeros(T.shape[0]))]:
                        for featurizer in [None, PolynomialFeatures(degree=2, include_bias=False)]:
                            for model_t, model_y, est_class, inference in [
                                (GradientBoostingClassifier(), GradientBoostingRegressor(), ForestDRLearner, 'auto'),
                                (
                                    LogisticRegression(solver='lbfgs', multi_class='auto'),
                                    LinearRegression(),
                                    LinearDRLearner,
                                ...
                                (
                                    ...
                                    LinearRegression(),
                                    LinearDRLearner,
                                    StatsModelsInferenceDiscrete(cov_type='nonrobust'),
                                ),
                                (
                                    LogisticRegression(solver='lbfgs', multi_class='auto'),
                                    LinearRegression(),
                                ...
                                with self.subTest(
                                    ...
                                    if X is not None:
                                        ...
                                        truth = np.hstack([1 + 0.5 * X[:3, [0]], 2 * (1 + 0.5 * X[:3, [0]])])
                                        TestDRLearner._check_with_interval(truth, point, lower, upper)
                                    else:
                                    ...
                                    if X is not None:
                                        if featurizer is not None:
                                            ...
                                            np.testing.assert_array_equal(est.featurizer_.n_features_in_, 2)
                                        np.testing.assert_array_equal(
                                            est.cate_feature_names(feature_names), out_feat_names
                                        )
                                    ...
                                    if isinstance(model_t, GradientBoostingClassifier):
                                        np.testing.assert_array_equal(
                                            ...
                                            [2, 2 + len(feature_names) + (W.shape[1] if W is not None else 0)],
                                        )
                                        np.testing.assert_array_equal(
                                            np.array(
                                                [mdl.feature_importances_ for mdl in est.models_propensity[0]]
                                            ).shape,
                                            [2, len(feature_names) + (W.shape[1] if W is not None else 0)],
                                        )
                                    else:
                                        np.testing.assert_array_equal(
                                            np.array([mdl.coef_ for mdl in est.models_regression[0]]).shape,
                                        ...
                                        np.testing.assert_array_equal(
                                            np.array([mdl.coef_ for mdl in est.models_propensity[0]]).shape,
                                            [2, 3, len(feature_names) + (W.shape[1] if W is not None else 0)],
                                        )
                                    ...
                                    if isinstance(est, ForestDRLearner):
                                        for t in [1, 2]:
                                            np.testing.assert_array_equal(
                                                est.feature_importances_(t).shape, [X.shape[1]]
                                            )
    ...
    def _check_with_interval(truth, point, lower, upper):
        np.testing.assert_allclose(point, truth, rtol=0, atol=0.2)
    ...
    def test_DRLearner(self):
        ...
        """
        DR_learner = DRLearner(model_regression=LinearRegression(), model_final=LinearRegression())
        # Test inputs
        ...
        DR_learner = DRLearner(
            model_regression=outcome_model, model_propensity=LogisticRegressionCV(), model_final=LinearRegression()
        )
    ...
    def _test_inputs(self, learner_instance):
        ...
        self.assertWarns(DataConversionWarning, learner_instance.fit, Y.reshape(-1, 1), T.reshape(-1, 1), X)
