...
from sklearn.linear_model import LinearRegression, LassoCV, Lasso, MultiTaskLasso, MultiTaskLassoCV, LogisticRegression
...
class TestDynamicDML(unittest.TestCase):
                        ...
                        for d_w in [2, None]:
                            ...
                            for featurizer, fit_cate_intercept in [
                                ...
                                coef_summaryframe_shape = (
                                    (d_y if d_y > 0 else 1) * (fd_x[0] if fd_x[0] > 0 else 1) * (d_t_final),
                                    6,
                                )
                                ...
                                est = DynamicDML(
                                    model_y=Lasso() if d_y < 1 else MultiTaskLasso(),
                                    model_t=LogisticRegression()
                                    if is_discrete
                                    else (Lasso() if d_t < 1 else MultiTaskLasso()),
                                ...
                                for inf in all_infs:
                                    with self.subTest(
                                        ...
                                        np.testing.assert_allclose(marg_eff if d_x else marg_eff[0:1], const_marg_eff)
                                        ...
                                        if inf is not None:
                                            ...
                                            self.assertEqual(
                                                shape(est.effect_interval(X, T0=T0, T1=T_test)), (2,) + effect_shape
                                            )
    ...
    def test_perf(self):
        ...
        # No heterogeneity
        dgp = DynamicPanelDGP(n_periods, n_treatments, n_x).create_instance(s_x, random_seed=12345)
        ...
        for test_idx in test_indices:
            est.fit(
                Y[test_idx],
                T[test_idx],
                X=X[test_idx] if X is not None else None,
                W=W[test_idx],
        ...
        for test_idx in test_indices:
            ...
            np.testing.assert_array_less(est.intercept__interval()[0], dgp.true_effect.flatten())
            np.testing.assert_array_less(dgp.true_effect.flatten(), est.intercept__interval()[1])
            np.testing.assert_array_less(est.coef__interval()[0] - 0.05, dgp.true_hetero_effect[:, hetero_inds + 1])
            np.testing.assert_array_less(dgp.true_hetero_effect[:, hetero_inds + 1] - 0.05, est.coef__interval()[1])
