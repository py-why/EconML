...
class Test2SLS(unittest.TestCase):
    ...
    def test_hermite_approx(self):
        ...
        y = (x[:, 1] > x[:, 0]) * (0 <= x[:, 0]) * (x[:, 0] <= 1) * (0 <= x[:, 1]) * (x[:, 1] <= 1) + np.random.normal(
            0, 0.01, (n,)
        )
                    ...
                    for d_z in [1, 2]:
                        ...
                        if d_z >= n_t:
                            ...
                            marginal_effect_shape = (
                                (n if d_x else 1,) + ((d_y,) if d_y > 0 else ()) + ((d_t,) if d_t > 0 else ())
                            )
    ...
    def test_2sls(self):
        ...
        for dt, dx, dz in [(0, 0, 0), (1, 1, 1), (5, 5, 5), (10, 10, 10), (3, 3, 10), (10, 10, 3)]:
            np2sls = SieveTSLS(
                t_featurizer=HermiteFeatures(dt),
