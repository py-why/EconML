...
class TestOrthoForest(unittest.TestCase):
    ...
    def test_continuous_treatments(self):
        ...
        for global_residualization in [False, True]:
            ...
            T = np.dot(
                TestOrthoForest.W[:, TestOrthoForest.support], TestOrthoForest.coefs_T
            ) + TestOrthoForest.eta_sample(TestOrthoForest.n)
            TE = np.array([self._exp_te(x) for x in TestOrthoForest.X])
            Y = (
                np.dot(TestOrthoForest.W[:, TestOrthoForest.support], TestOrthoForest.coefs_Y)
                + T * TE
    ...
    def test_binary_treatments(self):
        ...
        self.assertRaises(
            ValueError, est.fit, Y, np.ones((TestOrthoForest.n, 2)), X=TestOrthoForest.X, W=TestOrthoForest.W
        )
        # --> Check that it fails correctly when the treatments are not numeric
        self.assertRaises(
            ValueError, est.fit, Y, np.array(["a"] * TestOrthoForest.n), X=TestOrthoForest.X, W=TestOrthoForest.W
        )
        # Check that outputs have the correct shape
        out_te = est.const_marginal_effect(TestOrthoForest.x_test)
        ...
        est = DROrthoForest(
            ...
            bootstrap=False,
            n_jobs=1,
            propensity_model=LogisticRegression(C=1 / 0.024, penalty='l1', solver='saga'),
            model_Y=Lasso(alpha=0.024),
            propensity_model_final=LogisticRegressionCV(penalty='l1', solver='saga'),
            model_Y_final=WeightedLassoCVWrapper(),
        )
    ...
    def test_multiple_treatments(self):
        ...
        T = np.matmul(TestOrthoForest.W[:, TestOrthoForest.support], coefs_T) + uniform(
            -1, 1, size=(TestOrthoForest.n, 2)
        )
    ...
    def test_nuisance_model_has_weights(self):
        ...
        # Generate data with continuous treatments
        T = np.dot(TestOrthoForest.W[:, TestOrthoForest.support], TestOrthoForest.coefs_T) + TestOrthoForest.eta_sample(
            TestOrthoForest.n
        )
        TE = np.array([self._exp_te(x) for x in TestOrthoForest.X])
        Y = (
        ...
        est = DMLOrthoForest(n_jobs=1, n_trees=10, model_T=NoWeightModel(), model_Y=NoWeightModel())
        ...
        self.assertRaisesRegex(TypeError, weights_error_msg.format("NoWeightModel"), est.effect, X=TestOrthoForest.X)

    def _test_te(self, learner_instance, expected_te, tol, treatment_type='continuous'):
        # Compute the treatment effect on test points
        te_hat = learner_instance.const_marginal_effect(TestOrthoForest.x_test)
