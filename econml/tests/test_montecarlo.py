...
class TestMonteCarlo(unittest.TestCase):
    ...
    def test_discrete_treatment(self):
        """Test that we can perform nuisance averaging, and that it reduces the variance in a simple example."""
        y = np.random.normal(size=30) + [0, 1] * 15
        T = np.random.binomial(1, 0.5, size=(30,))
        W = np.random.normal(size=(30, 3))
        est1 = LinearDML(model_y=LinearRegression(), model_t=LogisticRegression(), discrete_treatment=True)
        est2 = LinearDML(model_y=LinearRegression(), model_t=LogisticRegression(), discrete_treatment=True, mc_iters=2)
        est3 = LinearDML(
    ...
    def test_parameter_passing(self):
        ...
        for gen in [LinearDML, SparseLinearDML, KernelDML, CausalForestDML]:
            est = gen(model_y=LinearRegression(), model_t=LinearRegression(), mc_iters=2, mc_agg='median')
            assert est.mc_iters == 2
            assert est.mc_agg == 'median'
