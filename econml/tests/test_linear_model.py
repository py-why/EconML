...
class TestLassoExtensions(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.y1 = (
            cls.intercept1
            + np.dot(cls.X[: cls.n_samples // 2], cls.coefs1)
            + np.random.normal(scale=cls.error_sd, size=cls.n_samples // 2)
        )
        cls.y2 = (
            cls.intercept2
            + np.dot(cls.X[cls.n_samples // 2 :], cls.coefs2)
            + np.random.normal(scale=cls.error_sd, size=cls.n_samples // 2)
        )
        ...
        cls.y_2D = np.concatenate(
            (TestLassoExtensions.y_simple.reshape(-1, 1), TestLassoExtensions.y.reshape(-1, 1)), axis=1
        )
        cls.y2_full = np.dot(cls.X, cls.coefs2) + np.random.normal(scale=cls.error_sd, size=cls.n_samples)
        cls.y_2D_consistent = np.concatenate(
            (TestLassoExtensions.y_simple.reshape(-1, 1), TestLassoExtensions.y2_full.reshape(-1, 1)), axis=1
        )
    ...
    def test_one_DGP(self):
        ...
        sample_weight = np.concatenate(
            (np.ones(TestLassoExtensions.n_samples // 2), np.ones(TestLassoExtensions.n_samples // 2) * 2)
        )
        ...
        self._compare_with_lasso(
            X_expanded,
            y_expanded,
            TestLassoExtensions.X,
            TestLassoExtensions.y_simple,
            sample_weight,
            alpha_range,
            params,
        )
        ...
        self._compare_with_lasso(
            X_expanded,
            y_expanded + TestLassoExtensions.intercept,
            TestLassoExtensions.X,
    ...
    def test_mixed_DGP(self):
        ...
        # Define weights
        sample_weight = np.concatenate(
            (np.ones(TestLassoExtensions.n_samples // 2), np.zeros(TestLassoExtensions.n_samples // 2))
        )
        # Data from one DGP has weight 0. Check that we recover correct coefficients
        self._compare_with_lasso(
            TestLassoExtensions.X[: TestLassoExtensions.n_samples // 2],
            TestLassoExtensions.y1,
            TestLassoExtensions.X,
            TestLassoExtensions.y,
            sample_weight,
        )
        # Mixed DGP scenario.
        sample_weight = np.concatenate(
        ...
        y_expanded = np.concatenate((TestLassoExtensions.y1, TestLassoExtensions.y2, TestLassoExtensions.y2))
        self._compare_with_lasso(X_expanded, y_expanded, TestLassoExtensions.X, TestLassoExtensions.y, sample_weight)
    ...
    def test_weighted_KFold(self):
        ...
        for _, test_index in wkf.split(
            TestLassoExtensions.X[:n], TestLassoExtensions.y_simple[:n], sample_weight=sample_weight
        ):
            # Compare fold weights
            self.assertAlmostEqual(np.sum(sample_weight[test_index]) / total_weight, 1 / n_splits, delta=5e-2)
    ...
    def test_balanced_weights_cv(self):
        ...
        wlasso_cv = list(
            cv_splitter.split(TestLassoExtensions.X, TestLassoExtensions.y_simple, sample_weight=sample_weight)
        )
        ...
        lasso_cv = self._map_splitter(
            wlasso_cv, TestLassoExtensions.n_samples + TestLassoExtensions.n_samples // 2, index_mapper
        )
        # Define alphas to test
        alphas = np.logspace(-4, -1, num=10)
        # Compare with LassoCV
    ...
    def test_multiple_outputs_balanced_weights_cv(self):
        ...
        cv_splitter = WeightedKFold(n_splits=3)
        wlasso_cv = list(
            cv_splitter.split(TestLassoExtensions.X, TestLassoExtensions.y_2D, sample_weight=sample_weight)
        )
        # Map weighted splitter to an extended splitter
        index_mapper = {}
        for i in range(TestLassoExtensions.n_samples):
    ...
    def test_debiased_lasso_one_DGP(self):
        ...
        expanded_debiased_coefs = self._check_debiased_coefs(
            X_expanded, y_expanded, sample_weight=None, expected_coefs=TestLassoExtensions.coefs1
        )
    ...
    def _check_debiased_CI(
        self, X, y, sample_weight, expected_coefs, expected_intercept=0, n_experiments=200, params={}
    ):
        # Unit vectors
    ...
    def _compare_with_lasso_cv(
        self, lasso_X, lasso_y, wlasso_X, wlasso_y, sample_weight, alphas, lasso_cv=3, wlasso_cv=3, params={}, tol=1e-8
    ):
        # Check if multitask
        if np.ndim(lasso_y) > 1:
...
class TestSelectiveRegularization(unittest.TestCase):
    ...
    def test_against_ridge_ground_truth(self):
        for _ in range(10):
            ...
            coefs = LinearRegression(fit_intercept=False).fit(np.vstack((X, X_aug)), np.concatenate((y, y_aug))).coef_
        ...
        for _ in range(10):
            ...
            models = [
                SelectiveRegularization(
                    ...
                    penalized_model=Lasso(alpha=alpha, fit_intercept=inner_intercept),
                    fit_intercept=True,
                )
    ...
    def test_vectors_and_arrays(self):
        ...
        model = SelectiveRegularization(unpenalized_inds=[0], penalized_model=Ridge(), fit_intercept=True)
    ...
    def test_can_use_sample_weights(self):
        ...
        model = SelectiveRegularization(unpenalized_inds=inds, penalized_model=Ridge(), fit_intercept=True)
        coef = model.fit(X, y, sample_weight=sample_weight).coef_
        coef2 = model.fit(np.vstack((X, X_aug)), np.concatenate((y, y_aug))).coef_
    ...
    def test_can_slice(self):
        ...
        y = np.random.normal(size=(n,))
        coef = (
            SelectiveRegularization(unpenalized_inds=slice(2, None), penalized_model=Lasso(), fit_intercept=True)
            .fit(X, y)
            .coef_
        )
        X_perm = np.hstack((X[:, 1:], X[:, :1]))
        coef2 = (
            SelectiveRegularization(unpenalized_inds=slice(1, -1), penalized_model=Lasso(), fit_intercept=True)
            .fit(X_perm, y)
            .coef_
        )
    ...
    def test_can_use_index_lambda(self):
        ...
        coef2 = (
            SelectiveRegularization(unpenalized_inds=index_lambda, penalized_model=Lasso(), fit_intercept=True)
            .fit(X, y)
            .coef_
        )
        np.testing.assert_allclose(coef, coef2)
    ...
    def test_can_pass_through_attributes(self):
        ...
        model = SelectiveRegularization(unpenalized_inds=[0], penalized_model=LassoCV(), fit_intercept=True)
