...
class TestModelSelection(unittest.TestCase):
    ...
    def test_poly(self):
        ...
        for true_d in range(1, 4):
            with self.subTest(true_d=true_d):
                ...
                mdl = LinearDML(model_t='poly', model_y=LinearRegression()).fit(y, t, X=X, W=W)
        ...
        for discrete_treatment in [True, False]:
            ...
            for model_t in ['auto', 'linear', 'poly', 'forest', 'gbf', 'nnet', 'automl']:
                with self.subTest(model_t=model_t, discrete_treatment=discrete_treatment):
                    mdl = LinearDML(model_t=model_t, discrete_treatment=discrete_treatment, model_y=LinearRegression())
                    mdl.fit(Y, T, X=X, W=W)
    ...
    def test_list_selection(self):
        ...
        mdl = LinearDML(model_t=[LinearRegression()], model_y=LinearRegression())
        mdl.fit(Y, T, X=X, W=W)

        # test corner case with empty list
        with self.assertRaises(Exception):
            mdl = LinearDML(model_t=[], model_y=LinearRegression())
            mdl.fit(Y, T, X=X, W=W)

        # test selecting between two fixed models
        mdl = LinearDML(model_t=[LinearRegression(), RandomForestRegressor()], model_y=LinearRegression())
        mdl.fit(Y, T, X=X, W=W)
        ...
        for is_discrete, mdls in [
            ...
            for mdl in mdls:
                ...
                with self.subTest(model=mdl):
                    est = LinearDML(model_t=mdl, discrete_treatment=is_discrete, model_y=LinearRegression())
    ...
    def test_fixed_model_scoring(self):
        ...
        mdl = LinearDRLearner(
            model_regression=SeparateModel(LassoCV(), LassoCV()), model_propensity=LogisticRegressionCV()
        )
        mdl.fit(Y, T, X=X, W=W)
        ...
        with self.assertRaises(Exception):
            mdl = LinearDRLearner(
                model_regression=[SeparateModel(LassoCV(), LassoCV()), Lasso()], model_propensity=LogisticRegressionCV()
            )
            mdl.fit(Y, T, X=X, W=W)
