...
def test_dominicks():
    ...
    class PriceFeaturizer(TransformerMixin):
        def __init__(
            self, n_prods, own_price=True, cross_price_groups=False, cross_price_indiv=True, per_product_effects=True
        ):
            base_arrays = []
            effect_names = []
            ...
            if per_product_effects:
                all = [
                    (np.diag(one_hots[p]) @ arr, nm + " {}".format(p))
    ...
    for name, op, xp_g, xp_i, pp in [
        ...
        (("Heterogeneous treatment effects" " with group effects"), True, True, False, True),
        (("Heterogeneous treatment effects" " with cross price effects"), True, False, True, True),
    ]:
        print(name)
        np.random.seed(42)

        ft = PriceFeaturizer(
            n_prods=3, own_price=op, cross_price_groups=xp_g, cross_price_indiv=xp_i, per_product_effects=pp
        )
        names = ft.names
        dml = LinearDML(model_y=RandomForestRegressor(), model_t=RandomForestRegressor(), featurizer=ft, cv=2)

        effects = []
        for store in stores:
            data = oj_data[oj_data['store'] == store].sort_values(by=['week', 'brand'])
            dml.fit(
                T=reshape(data.as_matrix(["logprice"]), (-1, 3)),
    ...
    np.random.seed(42)
    dml = LinearDML(model_y=RandomForestRegressor(), model_t=RandomForestRegressor(), featurizer=ConstFt(), cv=2)
