...
class TestDowhy(unittest.TestCase):
    def _get_data(self):
        ...
        T = np.random.binomial(1, 0.5, size=(250,))
    ...
    def test_dowhy(self):
        ...
        models = {
            "dml": LinearDML(model_y=reg(), model_t=clf(), discrete_treatment=True),
            "dr": DRLearner(model_propensity=clf(), model_regression=reg(), model_final=reg()),
            ...
            "orthoiv": OrthoIV(
                model_y_xw=reg(), model_t_xw=clf(), model_z_xw=reg(), discrete_treatment=True, discrete_instrument=False
            ),
            "dmliv": DMLIV(fit_cate_intercept=True, discrete_treatment=True, discrete_instrument=False),
            "driv": LinearDRIV(
        ...
        for name, model in models.items():
            with self.subTest(name=name):
                ...
                if name != "orf":
                    ...
                    est_dowhy.refute_estimate(
                        method_name="placebo_treatment_refuter", placebo_type="permute", num_simulations=3
                    )
                    est_dowhy.refute_estimate(method_name="data_subset_refuter", subset_fraction=0.8, num_simulations=3)
