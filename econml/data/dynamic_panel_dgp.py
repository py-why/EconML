...
def generate_coefs(index, columns):
    ...
    ind_investment = [columns.index(col) for col in columns if "investment" in col]

    for i in range(7):
        outcome_name = simulated_coefs_df.index[i]
        if "proxy" in outcome_name:
            ind_same_proxy = [ind for ind in ind_proxy if outcome_name in columns[ind]]
            # print(ind_same_proxy)
            random_proxy_name = np.random.choice([proxy for proxy in index[:4] if proxy != outcome_name])
            ind_random_other_proxy = [ind for ind in ind_proxy if random_proxy_name in columns[ind]]
            # demo
            simulated_coefs_df.iloc[i, np.random.choice(ind_demo, 2)] = np.random.uniform(0.004, 0.05)
            # same proxy
            simulated_coefs_df.iloc[i, ind_same_proxy] = sorted(
                np.random.choice(expon.pdf(np.arange(10)) * 5e-1, 6, replace=False)
            )
            simulated_coefs_df.iloc[i, ind_random_other_proxy] = sorted(
                np.random.choice(expon.pdf(np.arange(10)) * 5e-2, 6, replace=False)
            )
        elif "investment" in outcome_name:
            ind_same_invest = [ind for ind in ind_investment if outcome_name in columns[ind]]
            random_proxy_name = np.random.choice(index[:4])
            ind_random_other_proxy = [ind for ind in ind_proxy if random_proxy_name in columns[ind]]
            simulated_coefs_df.iloc[i, np.random.choice(ind_demo, 2)] = np.random.uniform(0.001, 0.05)
            simulated_coefs_df.iloc[i, ind_same_invest] = sorted(
                np.random.choice(expon.pdf(np.arange(10)) * 5e-1, 6, replace=False)
            )
            simulated_coefs_df.iloc[i, ind_random_other_proxy] = sorted(
                np.random.choice(expon.pdf(np.arange(10)) * 1e-1, 6, replace=False)
            )
...
def simulate_residuals(ind):
    ...
    if n_pos > 0:
        ...
        fitted_pos_outliers = sp.stats.lognorm(s, loc=loc, scale=scale).rvs(size=n_pos)
    else:
        fitted_pos_outliers = np.array([])
    # negative outliers
    if n_neg > 0:
        s, loc, scale = joblib.load(os.path.join(dir, f"input_dynamicdgp/lognorm_neg_{ind}.jbl"))
        fitted_neg_outliers = -sp.stats.lognorm(s, loc=loc, scale=scale).rvs(size=n_neg)
    else:
...
def get_prediction(df, coef_matrix, residuals, thetas, n, intervention, columns, index, counterfactual):
    ...
    pred_proxy = preds[:, :4] + sample_residuals[:, :4] + np.matmul(pred_inv, thetas.T)
    df[index[:4]] = pd.DataFrame(pred_proxy, index=df.index)
    return df


def generate_dgp(
    cov_matrix, n_tpid, t_period, coef_matrix, residual_matrix, thetas, intervention, columns, index, counterfactual
):
    df_all = pd.DataFrame()
    ...
    df = pd.DataFrame(
        np.hstack((np.arange(n_tpid).reshape(-1, 1), np.repeat(1, n_tpid).reshape(-1, 1), x)),
        columns=["id", "datetime"] + columns,
    )
    df = get_prediction(df, coef_matrix, residual_matrix[0], thetas, n_tpid, intervention, columns, index, False)
    ...
    for t in range(2, t_period + 1):
        ...
        df = get_prediction(
            new_df, coef_matrix, residual_matrix[t - 1], thetas, n_tpid, [0, 0, 0], columns, index, counterfactual
        )
...
class AbstracDynamicPanelDGP:
    ...
    def static_policy_effect(self, tau, mc_samples=1000):
        ...
        return np.mean(Y_tau[np.arange(Y_tau.shape[0]) % self.n_periods == self.n_periods - 1]) - np.mean(
            Y_zero[np.arange(Y_zero.shape[0]) % self.n_periods == self.n_periods - 1]
        )
...
class DynamicPanelDGP(AbstracDynamicPanelDGP):
    ...
    def create_instance(
        ...
        self.hetero_inds = hetero_inds.astype(int) if hetero_inds is not None else hetero_inds
        self.hetero_strength = hetero_strength
        self.autoreg = autoreg
        self.state_effect = state_effect
        self.random_seed = random_seed
        self.endo_inds = np.setdiff1d(np.arange(self.n_x), hetero_inds).astype(int)
        ...
        for t in range(self.n_x):
            self.Beta[t, :] = autoreg * np.roll(
                random_state.uniform(low=4.0 ** (-np.arange(0, self.n_x)), high=4.0 ** (-np.arange(1, self.n_x + 1))), t
            )
        if self.hetero_inds is not None:
        ...
        if self.hetero_inds is not None:
            self.y_hetero_effect[self.hetero_inds] = random_state.uniform(
                0.5 * hetero_strength, 1.5 * hetero_strength
            ) / len(self.hetero_inds)
            self.x_hetero_effect[self.hetero_inds] = random_state.uniform(
                0.5 * hetero_strength, 1.5 * hetero_strength
            ) / len(self.hetero_inds)
        ...
        self.true_hetero_effect[0, :] = cross_product(
            add_constant(self.y_hetero_effect.reshape(1, -1), has_constant='add'), self.epsilon.reshape(1, -1)
        )
        for t in np.arange(1, self.n_periods):
    ...
    def _gen_data_with_policy(self, n_units, policy_gen, random_seed=123):
        ...
        for t in range(n_units * self.n_periods):
            ...
            if period == 0:
                ...
                X[t] = (
                    (np.dot(self.x_hetero_effect, X[t - 1]) + 1) * np.dot(self.Alpha, T[t - 1])
                    + np.dot(self.Beta, X[t - 1])
                    + random_state.normal(0, self.sigma_x, size=self.n_x)
                )
                T[t] = policy_gen(T[t - 1], X[t], period, random_state)
            Y[t] = (
                (np.dot(self.y_hetero_effect, X[t]) + 1) * np.dot(self.epsilon, T[t])
    ...
    def observational_data(self, n_units, gamma, s_t, sigma_t, random_seed=123):
        ...
        def policy_gen(Tpre, X, period, random_state):
            return (
                gamma * Tpre + (1 - gamma) * np.dot(Delta, X) + random_state.normal(0, sigma_t, size=self.n_treatments)
            )
...
class SemiSynthetic:
    def create_instance(self):
        ...
        self.columns = [f"{ind}_{i}" for ind in self.index for i in range(-6, 0)] + [f"demo_{i}" for i in range(47)]
    ...
    def gen_data(self, n, n_periods, thetas, random_seed):
        ...
        for t in range(n_periods):
            sample_residuals = []
            for i in range(7):
                sample_residuals.append(random_state.choice(residual_matrix[:, i], n))
            sample_residuals = np.array(sample_residuals).T
            all_residuals.append(sample_residuals)
        all_residuals = np.array(all_residuals)

        fn_df_control = generate_dgp(
            self.cov_new, n, n_periods, coef_matrix, all_residuals, thetas, [0, 0, 0], self.columns, self.index, False
        )

        fn_df_cf_control = generate_dgp(
            self.cov_new, n, n_periods, coef_matrix, all_residuals, thetas, [0, 0, 0], self.columns, self.index, True
        )
        true_effect = np.zeros((n_periods, n_treatments))
        for i in range(n_treatments):
            intervention = [0, 0, 0]
            intervention[i] = 1
            fn_df_treated = generate_dgp(
                self.cov_new,
                n,
                n_periods,
                coef_matrix,
            ...
            for t in range(n_periods):
                ate_control = fn_df_cf_control.loc[fn_df_control["datetime"] == t + 1, outcome].mean()
                ate_treated = fn_df_treated.loc[fn_df_treated["datetime"] == t + 1, outcome].mean()
                true_effect[t, i] = ate_treated - ate_control

        new_index = ["proxy1", "proxy2", "proxy3", "proxy4"]
        new_columns = [f"{ind}_{i}" for ind in new_index for i in range(-6, 0)] + [f"demo_{i}" for i in range(47)]
        panelX = fn_df_control[new_columns].values.reshape(-1, n_periods, len(new_columns))
        panelT = fn_df_control[self.index[n_proxies:]].values.reshape(-1, n_periods, n_treatments)
