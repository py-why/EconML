...
class _BaseDMLIVNuisanceSelector(ModelSelector):
    ...
    def train(self, is_selecting, folds, Y, T, X=None, W=None, Z=None, sample_weight=None, groups=None, **fit_params):
        self._model_y_xw.train(
            is_selecting, folds, X, W, Y, **filter_none_kwargs(sample_weight=sample_weight, groups=groups, **fit_params)
        )
        self._model_t_xw.train(
            is_selecting, folds, X, W, T, **filter_none_kwargs(sample_weight=sample_weight, groups=groups, **fit_params)
        )
        # concat W and Z
        WZ = _combine(W, Z, Y.shape[0])
        self._model_t_xwz.train(
