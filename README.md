...
|---|---|---|---|
| Renamed constructor kwarg | `OneHotEncoder(sparse=...)` → `OneHotEncoder(sparse_output=...)` (1.2) | A small wrapper in `_sklearn_compat.py` that splats the right kwarg name based on a version flag | `one_hot_encoder()` in `econml/_sklearn_compat.py` |
| Renamed function kwarg | `force_all_finite=` → `ensure_all_finite=` on `check_array`/`check_X_y` (1.6; old name removed in 1.8) | A helper that returns the correct kwargs dict to splat into the call | `ensure_finite_kwargs()` in `econml/_sklearn_compat.py` |
| Symbol moved to a different (often private) submodule | `_get_column_indices` moved from `sklearn.utils` to `sklearn.utils._indexing` (1.5); `_print_elapsed_time` moved to `sklearn.utils._user_interface` (1.5) | A single conditional `import` in `_sklearn_compat.py` re-exports the symbol under a stable name; call sites import from there | `get_column_indices`, `print_elapsed_time` in `econml/_sklearn_compat.py` |
