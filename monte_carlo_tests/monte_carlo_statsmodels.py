...
def _coverage_profile(est, X_test, alpha, true_coef, true_effect):
    ...
    effect_interval = est.effect_interval(X_test, T0=np.zeros((X_test.shape[0], d_t)), T1=np.ones((X_test.shape[0], d_t)), alpha=alpha)
    ...
    cov['effect_stderr'] = est._model_final._model.prediction_stderr(cross_product(add_constant(X_test), np.ones((X_test.shape[0], d_t)))).flatten()
...
def plot_coverage(coverage, cov_key, n, n_exp, hetero_coef_list, d_list, d_x_list, p_list, t_list, cov_type_list, alpha_list, prefix="", folder="", print_matrix=False):
                    ...
                    for d_t in t_list:
                        for cov_type in cov_type_list:
                            for alpha in alpha_list:
                                key = "n_{}_n_exp_{}_hetero_{}_d_{}_d_x_{}_p_{}_d_t_{}_cov_type_{}_alpha_{}".format(n, n_exp, hetero_coef, d, d_x, p, d_t, cov_type, alpha)
                                if print_matrix:
...
def print_aggregate(mean_coverage, std_coverage, q_coverage, file_gen=lambda : None):
    for key, cov in mean_coverage.items():
        ...
        with np.printoptions(formatter={'float': '{:.2f}'.format}, suppress=True):
            print("Mean Coef (True, RMSE) \t (Coverage, Mean Length) \t Mean StdErr (True, RMSE)\t [Mean Lower (std), Mean Upper (std)]\t (True Quantiles)\r\n{}".format(
                "\r\n".join(["{:.2f} ({:.2f}, {:.2f}) \t ({:.2f}, {:.2f}) \t {:.2f} ({:.2f}, {:.2f}) \t [{:.2f} ({:.2f}), {:.2f} ({:.2f})] \t {}".format(est,
                                                                                true,
                                                                                np.sqrt(sqerr),
                                                                                coverage,
                                                                                length,
                                                                                stderr,
                                                                                true_stderr,
                                                                                np.sqrt(std_stderr**2 + (stderr - true_stderr)**2),
                                                                                lower,
                                                                                std_lower,
                                                                                upper,
                                                                                std_upper,
                                                                                true_qs) 
                                                            for est, true, sqerr, coverage, length, stderr, std_stderr, true_stderr, lower, std_lower, upper, std_upper, true_qs
                                                            in zip(cov['coef'],
                                                                   cov['true_coef'],
                                                                   cov['coef_sqerror'],
                                                                   cov['coef_cov'],
            ...
            print("Point\t Mean Coef (True, RMSE) \t (Coverage, Mean Length)\t Mean StdErr (True, RMSE)\t [Mean Lower (std), Mean Upper (std)]\t (True Quantiles)\r\n{}".format(
                "\r\n".join(["{}\t {:.2f} ({:.2f}, {:.2f}) \t ({:.2f}, {:.2f}) \t {:.2f} ({:.2f}, {:.2f}) \t [{:.2f} ({:.2f}), {:.2f} ({:.2f})] \t {}".format(','.join(x.astype(int).astype(str)),
                                                                                est,
                                                                                true,
                                                                                np.sqrt(sqerr),
                                                                                coverage,
                                                                                ...
                                                                                std_upper,
                                                                                true_qs) 
                                                            for x, est, true, sqerr, coverage, length, stderr, std_stderr, true_stderr, lower, std_lower, upper, std_upper, true_qs
                                                            in zip(cov['x_test'],
                                                                   cov['effect'],
                                                                   cov['true_effect'],
                                                                   cov['effect_sqerror'],
...
def run_all_mc(first_stage, folder, n_list, n_exp, hetero_coef_list, d_list, d_x_list, p_list, t_list, cov_type_list, alpha_list):
                    ...
                    for p in p_list:
                        for d_t in t_list:
                            ...
                            for it in range(n_exp):
                                ...
                                T = np.hstack([np.random.binomial(1, .5*X[:, 0]+.25, size=(n,)).reshape(-1,1) for _ in range(d_t)])
                                true_coef = np.hstack([np.hstack([it + np.arange(p).reshape(-1, 1), it + np.ones((p, 1)), np.zeros((p, d_x-1))])
                                                        for it in range(d_t)])
                                true_effect = lambda x, t: cross_product(np.hstack([np.ones((x.shape[0], 1)), x[:, :d_x]]), t) @ true_coef.T
                                y = true_effect(X, T) + X[:, [0]*p] + (hetero_coef*X[:, [0]] + 1)*np.random.normal(0, 1, size=(n,p))
                                ...
                                for cov_type in cov_type_list:
                                    ...
                                    lr = LinearDMLCateEstimator(model_y=first_stage(),
                                                        ...
                                                        n_splits=Splitter(),
                                                        linear_first_stages=False,
                                                        discrete_treatment=False).fit(y, X[:, -d_t:], X[:, :d_x], X[:, d_x:-d_t],
                                                                                    inference=StatsModelsInference(cov_type=cov_type))
                                    for alpha in alpha_list:
                                        key = "n_{}_n_exp_{}_hetero_{}_d_{}_d_x_{}_p_{}_d_t_{}_cov_type_{}_alpha_{}".format(n, n_exp, hetero_coef, d, d_x, p, d_t, cov_type, alpha)
    ...
    plot_coverage(agg_coverage_est, 'coef_cov', n, n_exp, hetero_coef_list, d_list, d_x_list, p_list, t_list, cov_type_list, alpha_list, prefix="sum_", folder=folder)
    plot_coverage(agg_coverage_lr, 'coef_cov',  n, n_exp, hetero_coef_list, d_list, d_x_list, p_list, t_list, cov_type_list, alpha_list, prefix="orig_", folder=folder)
    plot_coverage(agg_coverage_est, 'effect_cov',  n, n_exp, hetero_coef_list, d_list, d_x_list, p_list, t_list, cov_type_list, alpha_list, prefix="sum_", folder=folder)
    plot_coverage(agg_coverage_lr, 'effect_cov',  n, n_exp, hetero_coef_list, d_list, d_x_list, p_list, t_list, cov_type_list, alpha_list, prefix="orig_", folder=folder)
...
def monte_carlo(first_stage=lambda : LinearRegression(), folder='lr'):
    ...
    alpha_list = [.01, .05, .2]
    run_all_mc(first_stage, folder, n_list, n_exp, hetero_coef_list, d_list, d_x_list, p_list, t_list, cov_type_list, alpha_list)

def monte_carlo_lasso(first_stage=lambda : WeightedLasso(alpha=0.01, fit_intercept=True, tol=1e-6, random_state=123), folder='lasso'):
    n_exp = 1000
    n_list = [500]
    hetero_coef_list = [1]
    d_list = [20]
